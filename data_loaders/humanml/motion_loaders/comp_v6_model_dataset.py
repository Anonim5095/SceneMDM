    ...
    def __init__(self, args, model, diffusion, dataloader, mm_num_samples, mm_num_repeats, max_motion_length, num_samples_limit, scale=1.):
        ...
        with torch.no_grad():
            for i, (motion, model_kwargs) in tqdm(enumerate(dataloader)):
                ...
                keyframes = []
                if args.inpainting_mask == 'root_key' or args.inpainting_mask == 'root_horizontal' or args.inpainting_mask == "only_text":
                    ...
                    model_kwargs['y']['inpainting_mask'] = torch.tensor(get_inpainting_mask(args.inpainting_mask, motion.shape, model_kwargs['y']['lengths'], keyframes=None)).float().to(dist_util.dev())
                ...
                for t in range(repeat_times):
                    ...
                    for i,kf in enumerate(keyframes):
                        m = sample[i][:,0,:].T
                        for k in kf:
                            m[k][:259] = (m[k-1][:259] + m[k+1][:259]) / 2
                        new_sample.append(m.cpu().numpy())
                    new_sample = np.array(new_sample)

                    if t == 0:
                        sub_dicts = [{'gt_motion': motion[bs_i].squeeze().permute(1,0).cpu().numpy(),
                                    'motion': new_sample[bs_i],
                                    'length': model_kwargs['y']['lengths'][bs_i].cpu().numpy(),
                                    'caption': model_kwargs['y']['text'][bs_i],
                                    'tokens': tokens[bs_i],
                                    'cap_len': len(tokens[bs_i]),
